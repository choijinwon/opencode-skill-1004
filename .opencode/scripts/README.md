...
이 명령은 워크스페이스 루트의 `.ignore`, `.rgignore`, `.gitignore`에 관리 블록을 추가한다. 제외 대상은 `.venv/`, `node_modules/`, `.opencode/wsl/wheelhouse/`, `mlruns/`, `ai_studio/tracking/`, `saved_model/`, `*.pt`, `*.pkl`, `*.safetensors` 같은 생성물과 대용량 모델 파일이다.
