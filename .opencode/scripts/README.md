...
모델 선택 단계는 사용할 모델만 확정한다. 4번 템플릿 변환을 사용자가 선택하면 워크스페이스 루트 아래에 선택 모델명 작업 폴더를 만들고, `.opencode/samples/pytorch_sample/local_serving/` 폴더만 작업 폴더의 `local_serving/`로 복사한다. 이후 복사된 local serving 템플릿을 다시 읽고 선택 모델 기준으로 필요한 연결부만 최소 변환한다. `config/config.json`, `input_example.json`도 선택 모델 기준으로 변환한다. 템플릿의 `data/`와 `requirements.txt`는 복사하지 않고, 환경검증 단계에서 모델명 작업 폴더의 `requirements.txt`를 갱신한다.
...
Windows에서 MLflow에 업로드할 모델 원본 `path`는 Windows native 상대경로를 유지한다. 예: `saved_model\cnn_model.pt`. 모델 `url`과 MLflow artifact `uri` 값은 KServe/서버 해석 기준과 맞추기 위해 `saved_model/cnn_model.pt`, `config/config.json` 같은 POSIX(`/`) 형식으로 기록한다. MLmodel의 키 이름은 MLflow 스키마라 `uri`로 고정된다.
...
```text
1. 모델 목록 확인                  -> 01-project-analyze/validate_mlflow_project.py --no-write-check
...
```text
1 -> python .opencode/scripts/01-project-analyze/validate_mlflow_project.py --project . --no-write-check
...
워크스페이스 첫 분석은 아래 명령으로 실행한다. 1번 분석은 읽기 전용이며 `.env`와 `requirements.txt`를 생성하지 않는다.

```powershell
python .opencode/scripts/01-project-analyze/validate_mlflow_project.py --project . --no-write-check
```

`1~2`는 모델 목록 확인 -> 모델 선택 흐름이다. `3`은 환경 검증이며, 템플릿을 복사하지 않는다. `4` 템플릿 변환은 사용자가 선택했을 때 실행되며, `input_example.json`, `config/config.json`도 선택 모델 기준으로 변환한다. 템플릿 복사에서는 `local_serving/` 폴더만 복사한다.
필수 패키지 기준은 `03-environment-check/requirements.required.txt`에서 관리한다.

`3`은 모델 환경변수와 패키지 상태 체크다. 선택 모델명 작업 폴더에 `requirements.txt`가 없으면 필수 5개 기준으로 변환한다. `.env`의 `mlflow_tracking_uri`가 원격이면 원격 MLflow 서버 version을 확인해 `mlflow` 요구 버전을 변환하고, 선택 모델 `MODEL_KIND` 기준으로 필요한 프레임워크 패키지만 남긴다. 변환된 코드 import 기준 추가 Python 패키지가 필요하면 `requirements.txt`를 변환한다. 이때도 필수 패키지 5개는 절대 제거하지 않는다. 선택 모델명 작업 폴더에 `.env`가 없으면 MLflow 5개 키만 `""` 값으로 생성하고, 이미 있으면 복사하거나 덮어쓰지 않는다. MLflow 설정은 선택 모델명 작업 폴더의 `.env` 5개 값을 `set`, `empty`, `missing` 상태로만 표시한다. secret 값은 출력하지 않는다.
`mlflow_tracking_uri`, `mlflow_tracking_username`, `mlflow_tracking_password` 3개 값이 비어 있으면 4번 템플릿 변환과 5번 원격 MLflow 등록 실행은 차단한다. 사용자가 선택 모델명 작업 폴더의 `.env`에 직접 입력한 뒤 3번을 다시 실행해야 다음 단계로 넘어간다.
...
2. 현재 Python과 MLflow/requirements 호환성
...
```text
python .opencode/scripts/01-project-analyze/validate_mlflow_project.py --project . --no-write-check
python .opencode/scripts/01-project-analyze/validate_mlflow_project.py --project . --no-write-check --json
```
...
현재 Python, dependency, 원격 MLflow 서버 version, `.env` 상태를 확인한다.
Python은 고정 기대 버전으로 강제하지 않고, 원격 MLflow version과 requirements.txt 패키지 호환성 기준으로 확인한다.
...
환경검증은 현재 워크스페이스에 실제 존재하는 Python 파일들의 import를 확인해 누락된 Python 패키지를 `requirements.txt`에 추가한다. 대표 예시는 `runtest_2.py`, `aiu_custom/model.py`, `aiu_custom/predict.py`, `inferencetest.py`다. 단 선택 모델 `MODEL_KIND`와 무관한 프레임워크 패키지는 제거한다. 패키지 불일치/미설치가 있어도 로컬 dependency 설치는 자동 실행하지 않고, 상태와 필요 시 사용자 직접 설치 명령만 안내한다.
`mlflow_tracking_uri`이 있으면 원격 MLflow 서버의 `/version`을 확인하고, 서버 version과 로컬 `mlflow` 설치 version 및 `requirements.txt` 요구 version이 다르면 requirements.txt를 원격 서버 version 기준으로 변환하고 불일치로 표시한다.
Python 호환성 확인이 필요하면 `차단 항목 요약`에 다음 형식으로 표시한다.

```text
1. Python 버전 확인 (<현재버전>) → MLflow/requirements 호환성 기준으로 확인 필요
```
