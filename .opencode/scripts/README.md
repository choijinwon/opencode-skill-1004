...
모델 선택 단계에서 `.opencode/samples/pytorch_sample/` 내부 템플릿을 워크스페이스 루트로 복사한 뒤, `aiu_custom/`, `local_serving/`, `saved_model/`, `config/config.json`, `input_example.json`을 선택 모델 기준으로 준비한다. 템플릿의 `requirements.txt`는 복사하지 않고, 환경검증 단계에서 워크스페이스 루트 `requirements.txt`를 생성/갱신한다.
모델 선택 명령은 1~2번에서 선택 모델을 확정한다. 3번 환경검증을 실행하면 4번 템플릿 생성/변환이 자동 실행된다. `--sync-runtime`은 이미 생성된 `runtest_2.py` 기준으로 런타임 파일을 다시 맞출 때만 사용한다.
...
화면에 표시된 모델 번호나 TODO 단계 번호는 숫자 키로 입력하면 바로 선택/실행한다. 단, 4번 템플릿 변환은 사용자가 누르는 단계가 아니라 3번 완료 후 자동실행한다.
...
4 -> 3번 완료 후 자동실행: python .opencode\scripts\04-train-model\prepare_selected_model.py --project . --model selected --execute
...
`1~2`는 모델 목록 확인 -> 모델 선택 흐름이다. `3`은 환경변수/requirements 갱신이며, 이 단계가 끝나면 `4` 템플릿 생성/변환이 자동 실행된다. 템플릿 생성/변환에서는 `input_example.json`, `config/config.json`도 선택 모델 기준으로 준비한다. 템플릿 복사에서는 `requirements.txt`를 복사하지 않는다.
...
```text
python .opencode\scripts\qa-maintenance\test_local_sample.py --sample sklearn
python .opencode\scripts\qa-maintenance\test_local_sample.py --sample all
```
