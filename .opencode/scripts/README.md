...
환경검증은 현재 워크스페이스에 실제 존재하는 Python 파일들의 import를 확인해 누락된 Python 패키지를 `requirements.txt`에 추가한다. 대표 예시는 `runtest_2.py`, `aiu_custom/model.py`, `aiu_custom/predict.py`, `local_serving/localservingtest.py`다. 패키지 불일치/미설치가 있으면 기본 실행에서 `python -m pip install -r requirements.txt`를 실행한다.
