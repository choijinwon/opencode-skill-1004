...
SOURCE_MODEL_PATH = PROJECT_DIR / "saved_model" / "model.pkl"
DATA_MODEL_PATH = SOURCE_MODEL_PATH
MODEL_KIND = "sklearn_pickle"
MODEL_LOAD_HINT = "joblib.load(MODEL_PATH)"
...
def compute_metrics(model, test_x, test_y) -> dict[str, float]:
    ...
    return {
        ...
        "loss": float(log_loss(test_y, probability)),
    }
...
def main() -> None:
    ...
    config = {"framework": "sklearn", "input_dim": int(train_x.shape[1]), "output_dim": 2}
    ...
    with mlflow.start_run(run_name=mlflow_register_model_name) as run:
        mlflow.set_tag("data.name", "synthetic_tensor(sklearn)")
