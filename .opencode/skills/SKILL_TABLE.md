...
| 4 | `04-agent-mlflow-skill-train-model` | 모델 선택, `runtest_2.py` 생성, 템플릿 변환, 학습 실행 및 MLflow 등록 | `04-train-model/*` | `runtest_2.py`, `aiu_custom/`, `local_serving/`, `saved_model/` |
...
| 2 | 모델 선택 | `python .opencode/scripts/04-train-model/prepare_selected_model.py --project . --model <번호 또는 경로> --execute` | `aiu_custom/mapping.json` 기준 처음 선택 모델 고정 |
...
| 6 | 추론 테스트 | 사용자가 6번 선택 시 `python local_serving/localservingtest.py` | 입력/출력 추론 테스트 |
...
| 항목 | 요약 |
|---|---|
| 모델 선택 후 핵심 흐름 | 모델 목록 확인 -> 모델 선택 -> 템플릿 변환 -> 환경변수/requirements 갱신 -> 학습 실행 및 원격 MLflow 등록 -> 6번 선택 시 추론 테스트 |
