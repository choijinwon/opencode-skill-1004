...
| 3 | `03-agent-mlflow-skill-environment-check` | 모델 선택 후 환경 점검이 필요할 때 | `requirements.txt`, MLflow 입력값 | Python 버전, 패키지, MLflow 입력값, requirements 상태를 점검 | 환경 점검 결과, `requirements.txt` 갱신 |
...
| 6 | `06-agent-mlflow-skill-inference-test` | 사용자가 6번 추론 테스트를 선택했을 때 | `local_serving/localservingtest.py`, 입력 예시 | 로컬 추론 입력/출력 스키마와 predict 동작을 확인 | 추론 테스트 결과 |
...
| 6 | `06-agent-mlflow-skill-inference-test` | 추론 테스트 | 사용자가 6번 선택 시 `python .\local_serving\localservingtest.py` |
