...
4. 워크스페이스 루트 아래에 선택 모델명 작업 폴더를 만들고, `.opencode/samples/pytorch_sample/`의 `local_serving/`, `aiu_custom/`, `config/` 폴더를 복사한 뒤, 복사된 템플릿을 다시 읽고 선택 모델 기준 연결부만 최소 변환한다.
...
4. 템플릿 변환
   사용자가 4번 선택 시 실행. `local_serving/`, `aiu_custom/`, `config/` 폴더 복사 후, 복사된 템플릿 기준으로 선택 모델 경로와 모델 형식 연결부를 수정
5. 원격 MLflow 등록 실행
   사용자가 5번을 선택했을 때만 실행
6. 추론 테스트
   사용자가 6번을 선택했을 때만 실행
