...
# 분석용 주석
# ---------------------------------------------------------------------------
# 이 파일은 4단계 "샘플 부트스트랩"의 실제 핵심 스크립트 복사본입니다.
#
# 이 단계는 모델이 없거나 샘플 프로젝트를 새로 만들 때 사용합니다.
# 사용자가 선택한 샘플(sklearn / pytorch / tensorflow)을 기준으로
# `.opencode/samples/<sample>` 내부 파일을 현재 워크스페이스로 복사합니다.
#
# 중요한 원칙:
# 1) 사용자가 가진 `data/` 모델/데이터 파일은 복사하거나 덮어쓰지 않습니다.
# 2) 이미 존재하는 주요 파일은 가능한 한 보호하고, 필요한 기본 구조만 채웁니다.
# 3) 샘플 복사 단계이므로 모델 선택 후 변환하는 5단계 train-model과 역할이 다릅니다.
...
from common.ai_studio_process import format_todo_guide
...
def print_markdown_table(headers: list[str], rows: list[list[str]]) -> None:
    """
    분석 주석:
    - 단계 맥락: 4단계 샘플 부트스트랩: 모델이 없는 경우 선택 샘플을 워크스페이스로 복사하고 기존 파일을 보호합니다.
...
def has_required_dirs(sample: Path) -> bool:
    ...
    - 입력 기준: 입력값 `sample`를 기준으로 판단/변환을 수행합니다.
...
def iter_sample_files(sample: Path, skip_run_model: bool = False):
    ...
    - 입력 기준: 입력값 `sample`, `skip_run_model`를 기준으로 판단/변환을 수행합니다.
...
def is_scaffold_path(relative: Path) -> bool:
    ...
    - 단계 맥락: 4단계 샘플 부트스트랩: 모델이 없는 경우 선택 샘플을 워크스페이스로 복사하고 기존 파일을 보호합니다.
    - 함수 역할: 조건 판단용 boolean helper입니다. 큰 흐름의 분기 조건을 명확하게 분리합니다.
    - 입력 기준: 입력값 `relative`를 기준으로 판단/변환을 수행합니다.
    - 반환/효과: 조건 만족 여부를 True/False로 반환해 단계 분기에 사용합니다.
    """
...
def append_unique(items: list[str], item: str) -> None:
    ...
    - 단계 맥락: 4단계 샘플 부트스트랩: 모델이 없는 경우 선택 샘플을 워크스페이스로 복사하고 기존 파일을 보호합니다.
    - 함수 역할: 현재 단계의 세부 처리 로직을 함수 단위로 분리해 유지보수와 테스트가 쉽도록 합니다.
    - 입력 기준: 입력값 `items`, `item`를 기준으로 판단/변환을 수행합니다.
    - 반환/효과: 반환값보다 파일 생성, 콘솔 출력, 하위 명령 실행 같은 부수 효과가 핵심입니다.
    """
...
def copy_file(source: Path, target: Path) -> None:
    ...
    - 입력 기준: 입력값 `source`, `target`를 기준으로 판단/변환을 수행합니다.
    - 반환/효과: 반환값보다 파일 생성, 콘솔 출력, 하위 명령 실행 같은 부수 효과가 핵심입니다.
    """
...
def copy_existing_project_scaffold(sample: Path, project: Path, execute: bool) -> tuple[Path, list[str], list[str]]:
    ...
    - 단계 맥락: 4단계 샘플 부트스트랩: 모델이 없는 경우 선택 샘플을 워크스페이스로 복사하고 기존 파일을 보호합니다.
    - 함수 역할: 샘플 또는 템플릿 파일을 대상 워크스페이스로 복사하되 보호 규칙을 적용합니다.
    - 입력 기준: 입력값 `sample`, `project`, `execute`를 기준으로 판단/변환을 수행합니다.
...
def copy_sample(sample: Path, project: Path, force: bool, execute: bool, copy_mode: str) -> tuple[Path, list[str], list[str]]:
    ...
    - 단계 맥락: 4단계 샘플 부트스트랩: 모델이 없는 경우 선택 샘플을 워크스페이스로 복사하고 기존 파일을 보호합니다.
    - 함수 역할: 샘플 또는 템플릿 파일을 대상 워크스페이스로 복사하되 보호 규칙을 적용합니다.
    - 입력 기준: 입력값 `sample`, `project`, `force`, `execute`, `copy_mode`를 기준으로 판단/변환을 수행합니다.
    - 반환/효과: 여러 값을 tuple로 묶어 반환해 호출부가 상태, 경로, 오류 사유를 함께 판단하게 합니다.
    """
...
def build_tod_guide(target_project_path: Path, runtest_path: Path | None) -> list[str]:
    ...
    - 입력 기준: 입력값 `target_project_path`, `runtest_path`를 기준으로 판단/변환을 수행합니다.
...
def build_next_steps(sample_key: str, target_project_path: Path, runtest_path: Path | None) -> list[str]:
    ...
    - 단계 맥락: 4단계 샘플 부트스트랩: 모델이 없는 경우 선택 샘플을 워크스페이스로 복사하고 기존 파일을 보호합니다.
    - 함수 역할: 여러 검사 결과를 묶어 report, command, TODO guide 같은 최종 출력 구조를 생성합니다.
    - 입력 기준: 입력값 `sample_key`, `target_project_path`, `runtest_path`를 기준으로 판단/변환을 수행합니다.
    - 반환/효과: 후속 검사/출력에 사용할 목록을 반환합니다.
    """
